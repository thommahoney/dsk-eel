...
func InitSegments() [49]Segment {
	...
	// yellow - loop 1
	y1 := NewSegment("y1", offset)
	segments[0] = y1
	y1Start := NewStartHop(&y1)
	y1End := NewEndHop(&y1)
	offset += SegmentLength

	y2 := NewSegment("y2", offset)
	segments[1] = y2
	y2Start := NewStartHop(&y2)
	y2End := NewEndHop(&y2)
	offset += SegmentLength
	y3 := NewSegment("y3", offset)
	segments[2] = y3
	y3Start := NewStartHop(&y3)
	y3End := NewEndHop(&y3)
	offset += SegmentLength
	y4 := NewSegment("y4", offset)
	segments[3] = y4
	y4Start := NewStartHop(&y4)
	y4End := NewEndHop(&y4)
	offset += SegmentLength
	y5 := NewSegment("y5", offset)
	segments[4] = y5
	y5Start := NewStartHop(&y5)
	y5End := NewEndHop(&y5)
	offset += SegmentLength
	y6 := NewSegment("y6", offset)
	segments[5] = y6
	y6Start := NewStartHop(&y6)
	y6End := NewEndHop(&y6)
	offset += SegmentLength
	y7 := NewSegment("y7", offset)
	segments[6] = y7
	y7Start := NewStartHop(&y7)
	y7End := NewEndHop(&y7)
	offset += SegmentLength
	y8 := NewSegment("y8", offset)
	segments[7] = y8
	y8Start := NewStartHop(&y8)
	y8End := NewEndHop(&y8)
	offset += SegmentLength
	y9 := NewSegment("y9", offset)
	segments[8] = y9
	y9Start := NewStartHop(&y9)
	y9End := NewEndHop(&y9)
	offset += SegmentLength
	y10 := NewSegment("y10", offset)
	segments[9] = y10
	...
	// green - loop 2
	offset += SegmentLength
	g1 := NewSegment("g1", offset)
	segments[10] = g1
	g1Start := NewStartHop(&g1)
	g1End := NewEndHop(&g1)
	offset += SegmentLength
	g2 := NewSegment("g2", offset)
	segments[11] = g2
	g2Start := NewStartHop(&g2)
	g2End := NewEndHop(&g2)
	offset += SegmentLength
	g3 := NewSegment("g3", offset)
	segments[12] = g3
	g3Start := NewStartHop(&g3)
	g3End := NewEndHop(&g3)
	offset += SegmentLength
	g4 := NewSegment("g4", offset)
	segments[13] = g4
	g4Start := NewStartHop(&g4)
	g4End := NewEndHop(&g4)
	offset += SegmentLength
	g5 := NewSegment("g5", offset)
	segments[14] = g5
	g5Start := NewStartHop(&g5)
	g5End := NewEndHop(&g5)
	offset += SegmentLength
	g6 := NewSegment("g6", offset)
	segments[15] = g6
	g6Start := NewStartHop(&g6)
	g6End := NewEndHop(&g6)
	offset += SegmentLength
	g7 := NewSegment("g7", offset)
	segments[16] = g7
	g7Start := NewStartHop(&g7)
	g7End := NewEndHop(&g7)
	offset += SegmentLength
	g8 := NewSegment("g8", offset)
	segments[17] = g8
	g8Start := NewStartHop(&g8)
	g8End := NewEndHop(&g8)
	offset += SegmentLength
	g9 := NewSegment("g9", offset)
	segments[18] = g9
	g9Start := NewStartHop(&g9)
	g9End := NewEndHop(&g9)
	offset += SegmentLength
	g10 := NewSegment("g10", offset)
	segments[19] = g10
	...
	// blue - loop 3
	offset += SegmentLength
	b1 := NewSegment("b1", offset)
	segments[20] = b1
	b1Start := NewStartHop(&b1)
	b1End := NewEndHop(&b1)
	offset += SegmentLength
	b2 := NewSegment("b2", offset)
	segments[21] = b2
	b2Start := NewStartHop(&b2)
	b2End := NewEndHop(&b2)
	offset += SegmentLength
	b3 := NewSegment("b3", offset)
	segments[22] = b3
	b3Start := NewStartHop(&b3)
	b3End := NewEndHop(&b3)
	offset += SegmentLength
	b4 := NewSegment("b4", offset)
	segments[23] = b4
	b4Start := NewStartHop(&b4)
	b4End := NewEndHop(&b4)
	offset += SegmentLength
	b5 := NewSegment("b5", offset)
	segments[24] = b5
	b5Start := NewStartHop(&b5)
	b5End := NewEndHop(&b5)
	offset += SegmentLength
	b6 := NewSegment("b6", offset)
	segments[25] = b6
	b6Start := NewStartHop(&b6)
	b6End := NewEndHop(&b6)
	offset += SegmentLength
	b7 := NewSegment("b7", offset)
	segments[26] = b7
	b7Start := NewStartHop(&b7)
	b7End := NewEndHop(&b7)
	offset += SegmentLength
	b8 := NewSegment("b8", offset)
	segments[27] = b8
	b8Start := NewStartHop(&b8)
	b8End := NewEndHop(&b8)
	offset += SegmentLength
	b9 := NewSegment("b9", offset)
	segments[28] = b9
	...
	// red - loop 4
	offset += SegmentLength
	r1 := NewSegment("r1", offset)
	segments[29] = r1
	r1Start := NewStartHop(&r1)
	r1End := NewEndHop(&r1)
	offset += SegmentLength
	r2 := NewSegment("r2", offset)
	segments[30] = r2
	r2Start := NewStartHop(&r2)
	r2End := NewEndHop(&r2)
	offset += SegmentLength
	r3 := NewSegment("r3", offset)
	segments[31] = r3
	r3Start := NewStartHop(&r3)
	r3End := NewEndHop(&r3)
	offset += SegmentLength
	r4 := NewSegment("r4", offset)
	segments[32] = r4
	r4Start := NewStartHop(&r4)
	r4End := NewEndHop(&r4)
	offset += SegmentLength
	r5 := NewSegment("r5", offset)
	segments[33] = r5
	r5Start := NewStartHop(&r5)
	r5End := NewEndHop(&r5)
	offset += SegmentLength
	r6 := NewSegment("r6", offset)
	segments[34] = r6
	r6Start := NewStartHop(&r6)
	r6End := NewEndHop(&r6)
	offset += SegmentLength
	r7 := NewSegment("r7", offset)
	segments[35] = r7
	r7Start := NewStartHop(&r7)
	r7End := NewEndHop(&r7)
	offset += SegmentLength
	r8 := NewSegment("r8", offset)
	segments[36] = r8
	r8Start := NewStartHop(&r8)
	r8End := NewEndHop(&r8)
	offset += SegmentLength
	r9 := NewSegment("r9", offset)
	segments[37] = r9
	r9Start := NewStartHop(&r9)
	r9End := NewEndHop(&r9)
	offset += SegmentLength
	r10 := NewSegment("r10", offset)
	segments[38] = r10
	...
	// purple - loop 5
	offset += SegmentLength
	p1 := NewSegment("p1", offset)
	segments[39] = p1
	p1Start := NewStartHop(&p1)
	p1End := NewEndHop(&p1)
	offset += SegmentLength
	p2 := NewSegment("p2", offset)
	segments[40] = p2
	p2Start := NewStartHop(&p2)
	p2End := NewEndHop(&p2)
	offset += SegmentLength
	p3 := NewSegment("p3", offset)
	segments[41] = p3
	p3Start := NewStartHop(&p3)
	p3End := NewEndHop(&p3)
	offset += SegmentLength
	p4 := NewSegment("p4", offset)
	segments[42] = p4
	p4Start := NewStartHop(&p4)
	p4End := NewEndHop(&p4)
	offset += SegmentLength
	p5 := NewSegment("p5", offset)
	segments[43] = p5
	p5Start := NewStartHop(&p5)
	p5End := NewEndHop(&p5)
	offset += SegmentLength
	p6 := NewSegment("p6", offset)
	segments[44] = p6
	p6Start := NewStartHop(&p6)
	p6End := NewEndHop(&p6)
	offset += SegmentLength
	p7 := NewSegment("p7", offset)
	segments[45] = p7
	p7Start := NewStartHop(&p7)
	p7End := NewEndHop(&p7)
	offset += SegmentLength
	p8 := NewSegment("p8", offset)
	segments[46] = p8
	p8Start := NewStartHop(&p8)
	p8End := NewEndHop(&p8)
	offset += SegmentLength
	p9 := NewSegment("p9", offset)
	segments[47] = p9
	p9Start := NewStartHop(&p9)
	p9End := NewEndHop(&p9)
	offset += SegmentLength
	p10 := NewSegment("p10", offset)
	segments[48] = p10
...
func NewSegment(label string, offset int) Segment {
	return Segment{Label: label, Length: SegmentLength, Offset: offset}
}
